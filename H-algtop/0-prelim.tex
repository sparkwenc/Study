...
\begin{dfn}
    Let $\Dta^{n}$ be the standard $n$-simplex, and $\Dta$ the simplex category. A \tbf{simplicial set} $\cC$ is a presheaf of sets on the simplex category $\Dta$, denote by $\Set_{\Dta}$ the category of simplicial sets.
\end{dfn}
...
\begin{dfn}[2.1.1.8, 2.1.2.1]
    ...
    \begin{enr}[label=(\arabic*)]
        \item \tbf{inert} if for each $i\in\sA{n}\xC$, $\sP{f\xI\{i\}}=1$.
        \item \tbf{active} if $f\xI\{*\}=\{*\}$.
    \end{enr}
\end{dfn}
...
\begin{dfn}[2.1.2.3]
    ...
    \begin{enr}[label=(\arabic*)]
        \item \tbf{inert} if $p(f)$ is inert and $f$ is $p$-coCartesian.
        \item \tbf{active} if $p(f)$ is active.
    \end{enr}
\end{dfn}
