...
\title{\tbf{Diophantine approximation}}
\author{\tbf{Notes} by Cong Wen, 09/2023}
\date{}
% Last Updated: March 28, 2023
...
\thispagestyle{empty}
\begin{abstract}
	This is my self-use notes\footnote{Last updated on September 25, 2023} to understand some topics in \tbf{Diophantine approximation}.
...
This part of the note investigates \cite{Vojta1989Mordell,Faltings1991,Bombieri1990} around the Diophantine approximation method.



\sct{Arithmetic bigness and uniform Bogomolov}

This is the note for \cite{Yuan2021Bigness}.

\ssc{Canonical admissible metrics and Deligne pairing}


\prg{The Zhang metrics}
\begin{thm}[Theorem~A.1]
	...
	\begin{enr}
		...
		\]
		\item the residue map
		\[
			(\oga_{C/K}\ot_{\cO_{C}}\cO(x))|_{x}\ar K'
		\]
		is an isometry.
	\end{enr}
\end{thm}
...
\begin{thm}[Theorem~2.3]
	Let $k$ be $\bZ$ or a field, $S$ quasi-projective flat normal integral scheme over $k$, for a smooth relative curve $\pi:X\ar S$ of genus $g>0$, diagonal morphism $\Dta:X\ar X\tms_{S}X$, then
	\begin{enr}
	...
	Moreover, one has
	\begin{enr}
		\item The canonical isomorphism of line bundles $\oga_{X/S}\ar\Dta\xS\cO(-\Dta)$ induces an isomorphism of adelic line bundles $\oL{\oga}_{X/S}\ar\Dta\xS\oL{\cO}(-\Dta)_{a}$.
		\item The canonical isomorphism of line bundles
		\[
			p_{1*}\sA{\cO(\Dta),p_{2}\xS\oga_{X/S}}\ar\oga_{X/S}
		\]
		induces an isomorphism of adelic line bundles
		\[
			p_{1*}\sA{\oL{\cO}(\Dta)_{a},p_{2}\xS\oL{\oga}_{X/S,a}}\ar\oL{\oga}_{X/S,a}
		\]
	\end{enr}
\end{thm}

\prg{The Deligne pairing}
...
\begin{thm}[{\cite[Theorem~4.1.2]{YZ2021}}]
	Let $k$ be either $\bZ$ or a field, let $Y$ be a flat and essentially quasi-projective integral scheme over $k$, $f:X\ar Y$ a projective flat morphism of relative dimension $n$. Assume that $X$ is integral and $Y$ is normal, then the original Deligne pairing induces a symetric and multilinear functor
	\[
...
\ssc{Invariants on reduction graphs and the bigness}
...
\prg{Graph invariants and Cinkir's bound}
...
\begin{thm}[Theorem~3.1, Theorem~3.2]
	Further assume that $\pi:X\ar S$ is of maximal variation, then
	\begin{enr}
...
\begin{enr}
	\item \tbf{Key 1}
	\begin{thm}[Noether's formula, Theorem~3.3]
	...
	\begin{rmk}
		The proof is graph theoretical, many invariants involved can be heuristically understood via Kirchhoff's circuit laws in electrical network as in \cite{BF2006}.

		The bound of $\eps$-invariant is in \cite{BF2006}, the bound of $\vfi$-invariant is in \cite[Theorem~2.11]{Cinkir2011}.
	\end{rmk}
	\item \tbf{Key 4}
	\begin{thm}[Theorem~3.5]
...
\ssc{Height inequality and the uniform Bogomolov}
...
\begin{prf}[Sketch of the proof]
	\tbf{Part 1, estimate number of points:} Let $C$ be a smooth projective curve over an algebraically closed field $K$, $Z\sbs C^{m}$ a proper Zariski closed subset of codimension $d>0$, $\Sgm\sbs C(K)$ a finite subset such that $\Sgm^{m}\sbs Z(K)$, then $(\#\Sgm)^{d}\leq\Deg_{m\bt N}(Z)$, where $N$ is a line bundle of degree $1$ over $C$.
	...
	\]
	\red{Should the middle term of Lemma 4.3 at the bottom of Page 56 be actually $(nM)^{\Dim Z-1}\cdot(np_{i}\xS N)\cdot Z$?}

	hence $n^{m-d}\Deg_{M}(Z)\geq\Deg_{nM}(Z)\geq n^{m}$ and one has the desired result.

	\tbf{Part 2, height bound from bigness:} for some $m\geq1$, $m\bt\oL{L}$ is big over $X_{m}=X_{/S}^{m}$, hence there is a closed subset $Z$ of codimension $1$ in $X_{m}$ such that
	\[
...
\prg{The number field case $K=\bQ$:}
...
\prg{The function field case $K=k(t)$:}
...
\prg{The uniformity of constants}
The constants $c_{1},c_{2}$ depend on $(g,k(t))$ a priori, in fact, they only depend on $(g,\Char(k))$ by base change of the prime field, to prove the independence of $\Char(k)$, one needs the upgraded version of the height inequality:

\begin{thm}[Theorem~4.8]
	Let $S$ be a flat quasi-projective integral scheme over $\bZ$, $\psi:S\ar\bA_{\bZ}^{1}$ a flat morphism of finite type. Let $\pi:X\ar S$ be a smooth relative curve of genus $g\gqs2$, $\oL{L}$ a nef adelic line bundle over $X/\bZ$, $\oL{M}$ an adelic line bundle over $S/\bZ$, if $\oL{L}$ is potentially big over $X/S$, then there is an open subscheme $U$ of $S$, a integer $N'$, and constants $c_{1},c_{2}>0$, such that for any $K=k(t), \Char(k)\nmid N', y\in U_{k}(\oL{K})$,
	\[
		\#\{x\in X_{k}(\oL{K}):\pi(x)=y, h_{\oL{L}}(x)\leq c_{1}h_{\oL{M}}(y)\}\leq c_{2}.
	\]
\end{thm}

Assuming this, to check uniformity, one has to check for all but finitely many $\Char(k)$, fix $N\gqs3$, and $S=M_{g,N}$ the moduli space of smooth curves of genus $g$ over $\bZ[1/N]$ with full level-$N$ structure, $\pi:X\ar S$ the universal curve, similar to above, one only has to deal with the induction step: for any closed integral subscheme $Y$ of $J$ which is flat over $\bZ[1/N]$, there is an open subscheme $U$ of $Y$, a multiple $N'$ of $N$, and constants $c_{1},c_{2}>0$ such that for any $K=k(t)$ with $\Char(k)\nmid N'$, and non-isotrivial $y\in U_{k}(\oL{K})$ with $s=\pi_{J}(y)\in S_{k}(\oL{K})$,
\[
