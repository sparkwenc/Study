...
\end{enr}
\prg{Effective sections} Neukirch's $\#\HH{}{0}{(\cO_{K})}$ is the same as Abbes-Bouche's $\#\HH{\mathrm{Ar}}{0}{(X,L)}$ if $X=\Spc{\cO_{K}}$.
...
\prg{Arithmetic degree} I think $\oH{\chi}_{\mathrm{sup}}$ should really be thought of as arithmetic degree rather than the arithmetic Euler characteristic. I suspect it can be thought as the analogue of Euler characteristic because $\aHH{}{1}{}$ of number fields will actually vanish in my definition.

\prg{Ideas from QFT} What is the relationship between Kim's use of Chern-Simons functional and the Bott-Chern form I'm trying to understand? In particular, in that model $\Spc{\cO_{K}}$ should be $3$-dimensional, but from Arakelov theoretic point of view, it should be $1$-dimensional, what's the problem here?
