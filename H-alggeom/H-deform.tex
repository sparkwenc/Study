...
\title{\tbf{Deformation Theory} (reading notes)}
...
\thispagestyle{empty}
\begin{abstract}
	This is my reading notes\footnote{Last updated on February 08, 2024} on \tbf{Deformation Theory}\cite{Hartshorne2010}. Please reach out if you find anything incorrect.

\end{abstract}
\tableofcontents
% ----------------------------------------------------------------------------------------------------

\sct*{Cotangent complex}
\begin{stp}
...
\sct{Intro}
Four deformation problems are considered
\begin{enr}
	\item (A) subschemes of a fixed scheme $X$,
	\item (B) line bundles of a fixed scheme $X$,
	\item (C) coherent sheaves of a fixed scheme $X$,
	\item (D) deformation of abstract schemes.
\end{enr}

\sct{First-Order Deformations}
\begin{stp}
...
\begin{thm}
	...
	\begin{enr}
		\item (A) {\cite[Theorem~2.4]{Hartshorne2010}} Let $Y$ be a closed subscheme of $X$, then deformations is given by $\HH{}{0}{(Y,\cN_{Y/X})}$.
		\item (B) {\cite[Proposition~2.6]{Hartshorne2010}} Let $\cL$ be an invertible sheaf on $X$, then deformations is given by $\HH{}{1}{(X,\cO_{X})}$.
		\item (C) {\cite[Theorem~2.7]{Hartshorne2010}} Let $\cF$ be a coherent sheaf on $X$, then deformations is given by $\Ext{\cO_{X}}{1}{(\cF,\cF)}$.
		\item (D) {\cite[Theorem~5.3]{Hartshorne2010}} If $X$ is nonsingular, then the deformation of $X$ is given by $\HH{}{1}{(X,\cT_{X})}$.
	\end{enr}
\end{thm}
