...
\begin{dfn}[{\cite[4.3~Definition, 5.1]{MW1984}}]
	...
	\]
	\begin{itm}
		\item Define the \tbf{basic ideal} $\kb_{\km}^{(n)}\sbs R_{\km}^{(n)}$ to be the annihilator ideal of $C_{\km}^{(n)}$.
		\item Define the \tbf{stabilized Eisenstein ideal} $I_{\km}^{(n)}\sbs\bT_{\km}^{(n)}$ to be the annihilator ideal of $C_{\km}^{(n)}$.
	\end{itm}
